...
const runGame = original => {
    ...
    if (numClick == 0) {
        ...
        elOne.src = './img/' + gameState[clickValues[numClick] - 1].toString() + '.jpg';
        numClick++;
        ...
        if (clickValues[0] === clickValues[1]) {
            ...
            elTwo.src = './img/' + gameState[clickValues[numClick] - 1].toString() + '.jpg';
            numClick++;
    ...
    if (numberOfWins >= NUMBER_OF_PAIRS) {
        if (confirm(`Game is Over! Your Time Was: ${stopwatchEl.innerHTML}. Would you like to play again?`)) {
            newGame();
        } else {
            pauseTimer();
            return;
